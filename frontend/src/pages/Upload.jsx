  ...
  const uploadFile = (id, file, name) => {
    ...
    xhr.onload = () => {
      ...
      try {
        ...
        // non-JSON response, fall through to the HTTP-status check below
      }

      // The backend uploads one file per request here, so the first
      // entry in `files` reflects this specific upload. It can report
      // status: "error" per-file even though the overall HTTP response
      // is 200, e.g. if the DB insert itself failed for that file —
      // that must be treated as a failure too, not just a non-2xx status.
      const fileResult = result?.files?.[0];
      const dbInsertFailed = fileResult?.status === 'error';

      if (xhr.status >= 200 && xhr.status < 300 && !dbInsertFailed) {
        setFiles((prev) => prev.map((f) => (f.id === id ? { ...f, progress: 100, status: 'done' } : f)));
        showToast(`${name} uploaded successfully`, 'success');
      } else {
        setFiles((prev) => prev.map((f) => (f.id === id ? { ...f, status: 'error' } : f)));
        showToast(fileResult?.message || `${name} failed to upload`, 'error');
      }
    };
